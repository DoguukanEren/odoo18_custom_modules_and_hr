...
class HrEmployeePrivate(models.Model):
    ...
    # EKLENENLER
    extra_note = fields.Char(string="Ek Not") #Kısa Açıklama
    employment_start = fields.Date(string="İşe Başlama Tarihi") #Tarih girişi için direkt DATE seçildi
    experience_years = fields.Integer(int="Deneyim (Yıl)") #Normal Şartlarda String Daha Uygun
    is_certified = fields.Boolean(string="Sertifikalı mı?")# Boolean Değer Denemesi
    manager_note = fields.Text(string="Yönetici Notu", groups="hr.group_hr_manager")  # Sadece yöneticilerin görmesi için

    hobby = fields.Selection([
        ('reading', 'Kitap Okuma'),
        ('traveling', 'Seyahat Etme'),
        ('gaming', 'Oyun Oynama'),
        ('cooking', 'Yemek Yapma'),
        ('other', 'Diğer')
    ], string="Hobiler", default='gaming')
    hobby_level = fields.Selection(   # Selection: kullanıcı belirli seçenekler arasında seçim yapar Radio Button
        selection=[(str(i), str(i)) for i in range(1, 11)], #1 10 Puanlama için list yapısı
        string="Hobi Seviyesi"
    )
    hobby_cost = fields.Float(string="Hobiye Harcanan Ortalama Maliyet")  # Float Ondalık Sayı Örneği


    fobby = fields.Char(string="Fobisi") #Page Denemesi
